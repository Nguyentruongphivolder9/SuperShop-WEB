...
const ImageCropper = () => {
    ...
    const [imgSrc, setImgSrc] = useState<string | null>(profile?.avatarUrl ? profile.avatarUrl : null);
    ...
    return (
        <div className="flex flex-col md:flex-row justify-between space-y-4 md:space-y-0 md:space-x-4">
            <div className="bg-cyan-100 bg-opacity-40 rounded-md border-dashed border-2 border-sky-500 w-full p-8 flex justify-center items-center">
                        ...
                        {errMessage && (
                            <div className="flex items-center text-red-500 text-xs">
                                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m-2-4h.01M12 18h.01M21 12c0 4.97-4.03 9-9 9s-9-4.03-9-9 4.03-9 9-9 9 4.03 9 9z"></path>
                                </svg>
                                <p>{errMessage}</p>
                            </div>
                        )}
